...
PREFIX = "OmegaAI/"
FILES = [
    "mod_info.json", "omega_ai.version", "README.md", "CHANGELOG.md",
    "docs/DESIGN.md", "docs/TESTING.md", "docs/ART.md", "jars/OmegaAI.jar",
    ...
    "data/missions/omega_ai3_trial/descriptor.json", "data/missions/omega_ai3_trial/icon.png",
]
...
def inspect_payload(payload: dict[str, bytes]) -> str:
    ...
    require("modThreadId" not in mod and "modThreadId" not in version_file,
            "A forum topic does not exist yet. Update this rule when a topic exists.")
    ...
    descriptor = json.loads(payload["data/missions/omega_ai3_trial/descriptor.json"])
    ...
    require(payload[mod["logo"]] == payload["data/missions/omega_ai3_trial/icon.png"], "The mission icon is out of date.")
    with zipfile.ZipFile(io.BytesIO(payload["jars/OmegaAI.jar"])) as jar:
        ...
        require("data/missions/omega_ai3_trial/MissionDefinition.class" in classes, "The fleet trial class is missing.")
...
def main() -> None:
    ...
    tests = inspect_tests()
    output = args.verify or ROOT / "dist" / f"Omega-AI-{version}.zip"
    if not args.verify:

# Direct combat control in 0.2.0
...
`PilotSession` installs `OmegaShipAI` only over the exact native `BasicShipAI`, preserving the original pilot for restoration.
...
Publishing a proposal does not display a label. Expiry, manual takeover, loss of permission, changed controller, or Observe hides it.
Intents expire after 0.8 simulation seconds. Pausing does not consume their lifetime, but control checks still apply.
Mode changes and battle cleanup restore the original pilot only when the installed pilot still belongs to Omega.

...
public final class OmegaShipAI extends BasicShipAI {
    ...
    private boolean canSteer() {
        return !broken && pending != null && pending.expires() >= engine.getTotalElapsedTime(false)
                ...
                && ship.getShipAI() == this && permitted.getAsBoolean() && target != null && target.isAlive()
    ...
    @Override public void advance(float amount) {
        ...
        try {
            if (!canSteer() || isAvoidingCollision() || helm.blocked() || ship.getCommands().stream().anyMatch(Rc8Helm::system)) return;
            helm.steer(pending, amount, before);
            active = pending; // Only a consumed helm decision gets a label.
